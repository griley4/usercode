...
//int nybins = 10;
  ...
  float ptbins[]={0,8,10,16,20,25,50,80};
  float dybins[]={0,0.2,0.4,0.6,0.9,1.3,1.8,2.4,3.6};
...
// -- acceptance x efficiency shape
  ...
  disp->cd(1);
  gendps_tot = gendps.Draw("FourMu_Mass >> fourmu_mass","(FourMu_Mass>=0&&FourMu_Mass<=60&&Sum$(abs(Psi_y)<=1.9&&Psi_pT>8.0)==2&&Sum$(Psi_Mass[]<3.35&&Psi_Mass>2.85)==2)","");
  gendps_tot = gendps_tot*genscale;
  fourmu_mass.Scale(genscale);
  disp->cd(2);
  gendps_acc = gendps.Draw("FourMu_Mass >> fourmu_mass_alt2","(FourMu_Mass>=0&&FourMu_Mass<=60&&Sum$(abs(Psi_y)<=1.9&&Psi_pT>8.0)==2&&Sum$(abs(Mu_Eta)<=1.2&&Mu_pT>3.5||abs(Mu_Eta)>1.2&&abs(Mu_Eta)<1.6&&(Mu_pT>-3.75*abs(Mu_Eta)+8)||abs(Mu_Eta)>=1.6&&abs(Mu_Eta)<=2.0&&Mu_pT>2.0)==4&&Sum$(Psi_Mass[]<3.35&&Psi_Mass>2.85)==2)","");
  gendps_acc = gendps_acc*accscale;
  fourmu_mass_alt2.Scale(accscale);
  disp->cd(3);
  recodps_tot = recodps.Draw("FourMu_Mass >> fourmu_mass_alt","FourMu_Mass>=0&&FourMu_Mass<=60&&FourMu_VtxProb>=0.1&&Sum$(abs(Psi_y)<=1.9&&Psi_pT>8.0)==2&&Sum$(abs(Mu_Eta)<=1.2&&Mu_pT>3.5||abs(Mu_Eta)>1.2&&abs(Mu_Eta)<1.6&&(Mu_pT>-3.75*abs(Mu_Eta)+8)||abs(Mu_Eta)>=1.6&&abs(Mu_Eta)<=2.0&&Mu_pT>2.0)==4","");
