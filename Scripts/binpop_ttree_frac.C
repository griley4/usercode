...
//  dps.Add("Modified_Input_To_Fit_2012DPS_TMTight_3MuL3Match.root");
  ...
  float mincn = 0.82;
        ...
        dps.Draw("FourMu_pT >> pt_dps","FourMu_Mass>=0&&FourMu_Mass<=60&&FourMu_VtxProb>=0.1&&Sum$(abs(Psi_y)<=0.95&&Psi_pT>8.0||abs(Psi_y)>0.95&&abs(Psi_y)<1.4&&(Psi_pT>-5.55*abs(Psi_y)+13.3)||abs(Psi_y)>=1.4&&abs(Psi_y)<=1.9&&Psi_pT>5.5)==2&&Sum$(abs(Mu_Eta)<=1.2&&Mu_pT>3.5||abs(Mu_Eta)>1.2&&abs(Mu_Eta)<1.6&&(Mu_pT>-3.75*abs(Mu_Eta)+8)||abs(Mu_Eta)>=1.6&&abs(Mu_Eta)<=2.0&&Mu_pT>2.0)==4","goff");
        ...
        data.Draw("Psi1To2_dY >> dy_data","","goff");
        ...
        data.Draw("abs(FourMu_Rapidity)>> y_data","","goff");
        dps.Draw("abs(FourMu_Rapidity)>> y_dps","FourMu_Mass>=0&&FourMu_Mass<=60&&FourMu_VtxProb>=0.1&&Sum$(abs(Psi_y)<=0.95&&Psi_pT>8.0||abs(Psi_y)>0.95&&abs(Psi_y)<1.4&&(Psi_pT>-5.55*abs(Psi_y)+13.3)||abs(Psi_y)>=1.4&&abs(Psi_y)<=1.9&&Psi_pT>5.5)==2&&Sum$(abs(Mu_Eta)<=1.2&&Mu_pT>3.5||abs(Mu_Eta)>1.2&&abs(Mu_Eta)<1.6&&(Mu_pT>-3.75*abs(Mu_Eta)+8)||abs(Mu_Eta)>=1.6&&abs(Mu_Eta)<=2.0&&Mu_pT>2.0)==4","goff");
        ...
        std::cout<<" Mass Bins "<<std::endl;
        totN=dpsN=0;
        for(int mbin=1; mbin<=nmbins; ++mbin){
         totN =  a*scaledps*mass_dps->GetBinContent(mbin) + (1-a)*scalenlo*mass_nlogg->GetBinContent(mbin);
         dpsN = a*scaledps*mass_dps->GetBinContent(mbin);
         std::cout<<"dps_frac_mass->SetBinContent("<<mbin<<","<<dpsN/totN<<");"<<std::endl;
        }
        totN=dpsN=0;
        std::cout<<" pT Bins "<<std::endl;
        for(int ptbin=1; ptbin<=nptbins; ++ptbin){
         totN =  a*scaledps*pt_dps->GetBinContent(ptbin) + (1-a)*scalenlo*pt_nlogg->GetBinContent(ptbin);
         dpsN = a*scaledps*pt_dps->GetBinContent(ptbin);
         //std::cout<<"Bin "<<ptbin<<" DPS fraction: "<<dpsN/totN<<std::endl;
         std::cout<<"dps_frac_pt->SetBinContent("<<ptbin<<","<<dpsN/totN<<");"<<std::endl;
        }
        totN=dpsN=0;
        std::cout<<" Y Bins "<<std::endl;
        for(int ybin=1; ybin<=nybins; ++ybin){
         totN =  a*scaledps*y_dps->GetBinContent(ybin) + (1-a)*scalenlo*y_nlogg->GetBinContent(ybin);
         dpsN = a*scaledps*y_dps->GetBinContent(ybin);
         //std::cout<<"Bin "<<ybin<<" DPS fraction: "<<dpsN/totN<<std::endl;
         std::cout<<"dps_frac_y->SetBinContent("<<ybin<<","<<dpsN/totN<<");"<<std::endl;
        }
        totN=dpsN=0;
        std::cout<<" dY Bins "<<std::endl;
        for(int dybin=1; dybin<=ndybins; ++dybin){
         totN =  a*scaledps*dy_dps->GetBinContent(dybin) + (1-a)*scalenlo*dy_nlogg->GetBinContent(dybin);
         dpsN = a*scaledps*dy_dps->GetBinContent(dybin);
         //std::cout<<"Bin "<<dybin<<" DPS fraction: "<<dpsN/totN<<std::endl;
         std::cout<<"dps_frac_dy->SetBinContent("<<dybin<<","<<dpsN/totN<<");"<<std::endl;
        }
    ...
    dps.Draw("Psi1To2_dY>>DpsBestdy","(FourMu_Mass>=0&&FourMu_Mass<=60&&FourMu_VtxProb>=0.1&&Sum$(abs(Psi_y)<=0.95&&Psi_pT>8.0||abs(Psi_y)>0.95&&abs(Psi_y)<1.4&&(Psi_pT>-5.55*abs(Psi_y)+13.3)||abs(Psi_y)>=1.4&&abs(Psi_y)<=1.9&&Psi_pT>5.5)==2&&Sum$(abs(Mu_Eta)<=1.2&&Mu_pT>3.5||abs(Mu_Eta)>1.2&&abs(Mu_Eta)<1.6&&(Mu_pT>-3.75*abs(Mu_Eta)+8)||abs(Mu_Eta)>=1.6&&abs(Mu_Eta)<=2.0&&Mu_pT>2.0)==4)","goff");
