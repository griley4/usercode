...
int Xsec_calcTtreeFewerbins_Jul2Saved_PreDYCutoff() {
  gROOT->Reset();
...
tnp_diff_y->SetBinContent(9,  0.16372 );
  ...
  for(int ptbin=1; ptbin<=nptbins; ++ptbin){
    corrPoppt = pt_data->GetBinContent(ptbin)/(pow(0.0596,2)*20.2*(1+0.5*tnp_diff_pt->GetBinContent(ptbin))*((dps_frac_pt->GetBinContent(ptbin)*acc_eff_shape_dps_pt->GetBinContent(ptbin))+((1.-dps_frac_pt->GetBinContent(ptbin))*acc_eff_shape_nlo_pt->GetBinContent(ptbin)))*(ptbins[ptbin]-ptbins[ptbin-1]));
                std::cout<<pt_data->GetBinContent(ptbin)<<" over "<<0.0596*0.0596*20.3<<" times "<<dps_frac_pt->GetBinContent(ptbin)<<" times "<<acc_eff_shape_dps_pt->GetBinContent(ptbin)<<" + "<<(1-(dps_frac_pt->GetBinContent(ptbin)))<<" times "<<acc_eff_shape_nlo_pt->GetBinContent(ptbin)<<" times "<<(ptbins[ptbin]-ptbins[ptbin-1])<<" equals??? "<<corrPoppt<<" +/- relative "<<(pt_data->GetBinError(ptbin)/pt_data->GetBinContent(ptbin))<<std::endl;
//            std::cout<<pt_data->GetBinContent(ptbin)<<" over "<<0.0593*0.0593*20.339<<" times "<<fracDPS<<" times "<<acc_eff_shape_dps_pt->GetBinContent(ptbin)<<" + "<<fracNLO<<" times "<<acc_eff_shape_nlo_pt->GetBinContent(ptbin)<<" times "<<(ptbins[ptbin]-ptbins[ptbin-1])<<" equals??? "<<corrPoppt<<std::endl;
  ...
  for(int dybin=1; dybin<=ndybins; ++dybin){
    ...
    //          std::cout<<dy_data->GetBinContent(dybin)<<" over "<<0.0593*0.0593*20.2<<" times "<<((fracDPS*acc_eff_shape_dps_dy->GetBinContent(dybin))+(fracNLO*acc_eff_shape_nlo_dy->GetBinContent(dybin)))<<" times "<<(dybins[dybin]-dybins[dybin-1])<<" equals??? "<<corrPopdy<<std::endl;
