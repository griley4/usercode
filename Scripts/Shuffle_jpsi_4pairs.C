...
int main() {
  ...
  TChain *chainData = (TChain*)chainDataraw->CopyTree("FourMu_Mass>=0&&FourMu_Mass<=60&&FourMu_VtxProb>=0.1&&Sum$(abs(Psi_y)<=1.9&&Psi_pT>8.0)==2&&Sum$(abs(Mu_Eta)<=2.0&&Mu_pT>4.0)==4");

  TFile *fFile = new TFile("Shuffled_JPsi_sample_4pairs_JptGT8.root","recreate");
...
//      T2.SetM(cPsi_Mass[1]);
      for(int q=0; q<4; ++q){
        ...
        if(q==2){
          ...
          FourMu_Mass  = M2.M();
          FourMu_pT  = M2.Pt();
          FourMu_Rapidity  = M2.Rapidity();
          Psi_y[0] = T1.Rapidity();
          Psi_y[1] = P1.Rapidity();
          Psi_pT[0] = T1.Pt();
          Psi_pT[1] = P1.Pt();
          float diff1 = T1.Rapidity()-P1.Rapidity();
          Psi1To2_dY  = TMath::Abs(diff1);
          Psi1To2_dphi = TMath::Min((2*TMath::Pi()-TMath::Abs((TMath::ATan2(T1.Py(),T1.Px()))-(TMath::ATan2(P1.Py(),P1.Px())))),TMath::Abs((TMath::ATan2(T1.Py(),T1.Px()))-(TMath::ATan2(P1.Py(),P1.Px()))));
          Psi_pT_asym = (T1.Pt()-P1.Pt())/(T1.Pt()+P1.Pt());
          fTree->Fill();
        }
        if(q==3){
          TLorentzVector M3(0,0,0,0);
          M3 = P2 + T2;
          FourMu_Mass  = M3.M();
          FourMu_pT  = M3.Pt();
          FourMu_Rapidity  = M3.Rapidity();
          Psi_y[0] = T2.Rapidity();
          Psi_y[1] = P2.Rapidity();
          Psi_pT[0] = T2.Pt();
          Psi_pT[1] = P2.Pt();
          float diff1 = T2.Rapidity()-P2.Rapidity();
          Psi1To2_dY  = TMath::Abs(diff1);
          Psi1To2_dphi = TMath::Min((2*TMath::Pi()-TMath::Abs((TMath::ATan2(T2.Py(),T2.Px()))-(TMath::ATan2(P2.Py(),P2.Px())))),TMath::Abs((TMath::ATan2(T2.Py(),T2.Px()))-(TMath::ATan2(P2.Py(),P2.Px()))));
          Psi_pT_asym = (T2.Pt()-P2.Pt())/(T2.Pt()+P2.Pt());
          fTree->Fill();
        }
      }
      ++j;
